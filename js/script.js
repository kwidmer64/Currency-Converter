...
let currency = {
  ...
  displayConversion: function (result) {
    ...
    document.querySelector(".currency-from").innerText =
      amount + " " + currencyFrom + " equals";
    document.querySelector(".currency-to").innerText =
      result.result + " " + currencyTo;
  },
};
...
let symbols = {
  getSymbols: function () {
    //retrieves currency symbols in json format
    fetch("https://api.apilayer.com/exchangerates_data/symbols", requestOptions)
      .then(async function (response) {
        var json = await response.json();
        return json;
      })
      .then((result) => this.symbolOptionCreation(result))
      .catch((error) => console.log("error", error));
  },
  //creates dropdown selections for each currency symbol
  symbolOptionCreation: function (symbolInput) {
...
//gets symbols when page is loaded
window.onload = (event) => {
  symbols.getSymbols();
};
